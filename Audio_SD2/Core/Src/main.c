...
/**
  ...
  *
  * Opción A:
  *  - PA4 en ANALÓGICO (Hi-Z) al encender para que no suene nada.
  *  - SOLO MELODIA2: se reproduce en bucle cuando PC2 hace flanco H->L.
  *  - Entradas SIN pull interno (uso pull-ups externos).
  ******************************************************************************
  */
...
/* ===== Audio ===== */
static uint16_t audio_buf[AUDIO_BUF_SAMPLES]; /* buffer circular de DMA */
static uint16_t wavetable[WT_SIZE];           /* wavetable seno 12 bits */
...
/* UART debug: así me imprimo mensajes sin bloquear demasiado */
static void log_uart(const char* fmt, ...)
{
  char buf[160];
...
/************ Audio ************/
/* Genero tabla seno de 256 muestras en rango 0..4095 */
static void WT_InitSine(void) {
...
/* Ajusto incremento de fase (frecuencia). 0 => silencio */
static inline void Audio_SetFrequency(float freq_hz) {
...
/* Callback de relleno: mitad de buffer */
static void FillHalf(u32 offset, u32 len) {
  if (phase_inc_q16 == 0) {
...
/* Inicio del pipeline: DAC + DMA + TIM6 */
static void Audio_Start(void) {
...
/************ Player NO BLOQUEANTE ************/
/* Duración efectiva de una nota según tempo */
static inline uint32_t NoteDurMs(const Note* n, uint32_t tempo_ms)
{
...
static void MelodyStart(const Note* mel, unsigned count, uint32_t tempo_ms)
{
  ...
  Audio_SetFrequency(cur_mel[0].freq_hz);   /* cambio inmediato a la 1ª nota */
  if (!audio_running) Audio_Start();
  stage_end_ms = now + NoteDurMs(&cur_mel[0], cur_tempo);
}

/* Servicio periódico: avanza nota/gap y reinicia si hay loop */
static void MelodyService(void)
{
...
/* Leo un CSV (freq,dur) y lleno el arreglo de Note */
static FRESULT LoadMelodyFromCSV(const char* path, Note* out_buf, size_t max_notes, unsigned* out_count)
{
  *out_count = 0;
...
/* Intento abrir por varias rutas posibles */
static FRESULT TryOpenList(const char* const* list, size_t n, Note* buf, unsigned* out_cnt)
{
...
/************ Entradas (debounce) ************/
/* Confirmo LOW estable durante 'ms' para considerar flanco */
static uint8_t DebounceReadLow(GPIO_TypeDef* port, uint16_t pin, uint32_t ms)
{
...
/* Confirmo HIGH estable durante 'ms' para armar el disparo */
static uint8_t DebounceReadHigh(GPIO_TypeDef* port, uint16_t pin, uint32_t ms)
{
...
int main(void)
{
  ...
  MX_GPIO_Init();            /* PA4 en ANALOG Hi-Z aquí (silencio al boot) */
  MX_USART2_UART_Init();
  MX_SPI2_Init();
  ...
  /* Cargo SOLO MELODIA2 (o respaldo) — NO reproducir aún */
  if (SD_Mount() == FR_OK) {
  ...
  while (1) {
    ...
    if (!started) {
      if (!armed) {
        ...
        if (DebounceReadLow(TRIG1_GPIO_Port, TRIG1_Pin, DEBOUNCE_MS_LO)) {
          ...
          /* Arranca audio y loop */
          MelodyStart(mel, cnt, CSV_TEMPO_MS);
          loop_enabled = 1; loop_mel = mel; loop_len = cnt;
    ...
    /* Servicio de reproducción (reinicia si loop_enabled=1) */
    MelodyService();

    /* Diagnóstico cada ~500 ms */
...
/* ==== Resto: generado por CubeMX (mantengo estructura) ==== */
...
static void MX_DAC_Init(void)
{
  ...
  /* Lo reconfiguro a TIM6 TRGO justo antes de sonar */
