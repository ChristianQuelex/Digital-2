...
#define slave1 0x30		// Esclavo ADC (SW420)
...
int main(void)
{
    ...
    while (1) 
    {
		//----> Lectura del Sensor SW420 (vibración)
		PORTB|=(1 << PORTB5);		
		I2C_Master_Start();
		...
		if(temp != 1){
			...
			Sensor_SW40 = (valorI2C*75.0)/255.0;					
			float_to_string(Sensor_SW40, SW40VALOR_CHAR, 1);	

			LCD_Set_Cursor(1,2);
			LCD_Write_String(SW40VALOR_CHAR);				
			LCD_Set_Cursor(5,2);
			LCD_Write_String("Hz");
		}
